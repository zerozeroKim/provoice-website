...
const isCustomer = import.meta.env.VITE_SITE_MODE === 'customer';
  ...
  return (
    <div className={styles.bar} role="note" aria-label="사이트 디자인 버전 전환">
      ...
      <div className={styles.switch} role="group" aria-label="버전 선택">
        <button type="button" className={isCustomer ? styles.active : ''} aria-current={isCustomer} onClick={() => !isCustomer && goTo(PRODUCTION_URL)}>VER 1</button>
        <button type="button" className={!isCustomer ? styles.active : ''} aria-current={!isCustomer} onClick={() => isCustomer && goTo(DEV_URL)}>VER 2</button>
      </div>
    </div>
  );
}
