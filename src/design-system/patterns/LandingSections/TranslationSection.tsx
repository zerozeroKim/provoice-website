    ...
    <section className={styles.translationSection} aria-label="번역 서비스">
      ...
      <div className={styles.translationBody}>
        <div className={styles.translationMain}>
          <div className={styles.infoGrid}>
            <article className={`${styles.infoCard} ${styles.infoCardPrimary}`}>
              ...
              <div><span className={styles.cardOverline}>QUALITY STANDARD</span><strong>ISO 17100 인증</strong><p>번역·검수·최종 품질관리까지 국제 표준에 맞춰 운영합니다.</p></div>
            </article>
            <article className={styles.infoCard}>
              <div className={styles.cardIcon}><Languages /></div>
              <div><span className={styles.cardOverline}>GLOBAL NETWORK</span><strong>30개국 언어 지원</strong><p>현지 언어와 문화에 익숙한 원어민 전문가가 참여합니다.</p></div>
            </article>
            <article className={styles.infoCard}>
              <div className={styles.cardIcon}><Sparkles /></div>
              <div><span className={styles.cardOverline}>SMART WORKFLOW</span><strong>AI × Human 검수</strong><p>속도는 높이고, 표현의 자연스러움은 사람이 지킵니다.</p></div>
            </article>
          </div>
        ...
        <div className={styles.translationTable}>
          <div className={styles.tableHead}><div><span>SERVICE SCOPE</span><strong>번역 서비스 구성</strong></div><span>전 과정 원스톱</span></div>
          {rows.map((row, index) => (
